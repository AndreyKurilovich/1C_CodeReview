...
&НаКлиенте
Процедура УсловияВариантПоискаПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ВариантПоиска <> ПредопределенноеЗначение("Перечисление.ВариантыПоиска.Содержит") Тогда
		ТекущиеДанные.МеткаВКоде = Ложь;	
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Процедура УсловияМеткаВКодеПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.Условия.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Или ТекущиеДанные.МеткаВКоде = Ложь Тогда
		Возврат;
	КонецЕсли;

	Для Каждого УсловиеПоиска Из Объект.Условия Цикл
		
		Если ТекущиеДанные = УсловиеПоиска Тогда
			Продолжить;
		КонецЕсли;
		
		УсловиеПоиска.МеткаВКоде = Ложь;
